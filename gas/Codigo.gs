...
const HDR_GASTOS   = ['ID','Cliente','Tipo de Servicios/Proyecto','Concepto','Mes','Tipo de Servicio','Categoria','Monto','Solicitado por'];
...
function handlePost(e) {
  ...
  if (accion === 'saveGasto') {
    ...
    const row = [id, b.cliente||'', b.tipo||'', b.concepto||'', b.mes||'', b.tipoServicio||'',
      b.categoria||'', num(b.monto), b.solicitadoPor||''];
    const found = findRow(sh, id);
  ...
  if (accion === 'saveGastoBulk') {
    ...
    const base = new Date().getTime();
    const rows = items.map((b, i) => [
      b.id || ('G' + (base + i)), b.cliente||'', b.tipo||'', b.concepto||'', b.mes||'', b.tipoServicio||'',
      b.categoria||'', num(b.monto), b.solicitadoPor||''
    ]);
